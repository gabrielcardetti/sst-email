...
export default $config({
  ...
  async run() {
    ...
    // TODO: if domain is registered on Route53, add MX record to the domain (this is for incoming emails)
    ...
    new aws.ses.ActiveReceiptRuleSet("activeEmailRuleSet", {
    ...
    const api = new sst.aws.Function("sst-email", {
      ...
      permissions: [
        {
          actions: ["ses:SendEmail", "ses:SendRawEmail", "ses:CreateCustomVerificationEmailTemplate", "ses:CreateEmailIdentity"],
          resources: ["*"],
        },
        {
